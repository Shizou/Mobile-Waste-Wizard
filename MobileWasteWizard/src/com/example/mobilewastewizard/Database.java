...
public class Database 
{
	 ...
	 * @param cxt the context from which this
	 *  class acquires privileges for the assets folder (in this case 
	 *  main activity)*/
	public Database(Context cxt) 
	{
		...
		this.categoriesName[1] = "Transfer Station or Waste Depot";
			...
			for(int j = 0;j < M;j++)
			{
				...
				else
				{
					dp[i][j] = Math.min(Math.min(dp[i-1][j]+1, dp[i][j-1]+1),
							            dp[i][j]);
					dp[i][j] = Math.min(dp[i-1][j-1]+diff(A,B,i-1,j-1),
										dp[i][j]);
				}
			}
		}
	 ...
	 * Uses binary search so find the query in O(logN)
